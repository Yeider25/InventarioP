...
function obtenerDetallesSolicitud(id) {
    $.ajax({
        url: '../almacen/PHP/obtener_detalles_solicitud.php',
        method: 'GET',
        data: { solicitudId: id },
        success: function(response) {
            ...
            $('#cantidad_solicitada').val(datos.cantidad_solicitada);
            ...
            $('#solicitudId').val(id);
                console.log("ID de solicitud asignado: " + id);
        },
        error: function() {
...
$('.btn-ver-detalles').on('click', function() {
    const solicitudId = $(this).data('id');
    console.log('ID de solicitud obtenido:', solicitudId); // Verifica el ID aquí
    obtenerDetallesSolicitud(solicitudId);
});
        ...
        for (var i = 0, f; f = files[i]; i++) {
            ...
            reader.onload = (function(theFile) {
                return function(e) {
                    ...
                    $.ajax({
                        url: '../almacen/PHP/guardarFirma.php',
                        method: 'POST',
                        data: formData,
                        contentType: false,
                        processData: false,
...
document.getElementById('EnviarAceptacion').addEventListener('click', () => {
    const solicitudId = document.getElementById('solicitudId').value;
    console.log('ID de solicitud enviado:', solicitudId); // Verifica el ID aquí
    if (solicitudId) {
        $.ajax({
            url: '../almacen/PHP/RegistrarSolicitudAlmacen.php',
            type: 'POST',
        ...
        Swal.fire('Advertencia', 'No se encontró el ID de la solicitud.', 'warning');
    }
});