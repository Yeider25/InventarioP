...
// Verificar si se proporcionó el ID de la solicitud
if (!isset($_GET['solicitudId']) || empty($_GET['solicitudId'])) {
    die(json_encode(["error" => "⚠️ No se proporcionó un ID de solicitud válido."]));
}

$solicitudId = $_GET['solicitudId'];

// Consulta SQL para obtener los detalles de la solicitud
$query = "SELECT sp.id AS id_solicitud, sp.fecha_soli, sp.area, sp.cargo, 
                 esp.id_elemento, esp.cantidad_solicitada, esp.observaciones,
                 e.descripcion, e.und_medida
          FROM solicitud_periodica sp
          LEFT JOIN elementos_solicitud_periodica esp ON sp.id = esp.id_solicitud
          LEFT JOIN elemento e ON esp.id_elemento = e.id_elemento
          WHERE sp.id = ?";
...
if (!$stmt) {
    die(json_encode(["error" => "⚠️ Error en la consulta SQL: " . $conexion->error]));
}
...
while ($row = $result->fetch_assoc()) {
    $detalles[] = [
        ...
        "descripcion" => $row['descripcion'],
        "unidad_medida" => $row['und_medida'],
        "cantidad" => isset($row['cantidad_solicitada']) ? $row['cantidad_solicitada'] : 0,
        "observacion" => isset($row['observaciones']) ? $row['observaciones'] : "Sin observaciones"
    ];
}
